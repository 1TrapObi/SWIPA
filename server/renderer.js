...
function loadPage(event) {
    // view.loadURL("https://shop.havenshop.com/517042/checkouts/0a0017872cfdd833a8039d81613f23f1");
}

function updateNav(event) {
    omni.value = view.src;
    setTimeout(function () {
        view.sendInputEvent({ type: 'mouseDown', x: 70, y: 70, button: 'left', clickCount: 1 });
        view.sendInputEvent({ type: 'mouseUp', x: 70, y: 70, button: 'left', clickCount: 1 });
    }, 5000);
}

view.addEventListener('did-finish-load', updateNav);
view.addEventListener('dom-ready', loadPage);
